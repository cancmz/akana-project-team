...
/* GECICI TEST: UART5'e (TTL) ham veri + Aci X karsilastirma ciktisi.
   GPS'i gecici sokup TTL takman gerekiyor -- bu, sadece dogrulama
   testi icin, GPS ile ayni anda kullanilamaz. */
void Diag_Print(const char *str)
{
...
typedef struct {
    ...
    /* YENI: GAA artik aci esigine degil, ATE'den (alcalma tespiti) sonra
       RASTGELE bir gecikmeye bagli. Gecikme, FSM_Init'te BIR KEZ secilir
       (0.8-1.5sn arasi, 0.1sn adimlarla -- yani 8,9,10,...,15 * 100ms). */
    uint32_t gaa_delay_ms;
    uint32_t ate_confirmed_time_ms;
} recovery_fsm_t;
...
static void FSM_Init(recovery_fsm_t *f, recovery_config_t cfg)
{
    ...
    /* YENI: her FSM baslatildiginda (yeni test/ucus), 0.8-1.5sn arasi,
       0.1sn adimlarla RASTGELE bir GAA gecikmesi secilir -- 8 olasi
       deger (8,9,10,11,12,13,14,15 -> *100ms = 800..1500ms). */
    int steps = 8 + (rand() % 8); /* 8..15 */
    f->gaa_delay_ms = (uint32_t)steps * 100u;
}

static uint16_t FSM_Update(recovery_fsm_t *f, float irtifa, float ivme_z, float tilt_deg)
{
    (void)tilt_deg; /* ARTIK KULLANILMIYOR -- GAA artik aci esigine degil,
                        ATE sonrasi rastgele gecikmeye bagli (asagida). */
    ...
    if (f->status_bits & BIT_IEA) {
        /* ATE ONCE kontrol ediliyor -- GAA artik BUNA bagli bir zamanlayici. */
        if (!f->ate_confirmed && f->have_prev_irtifa) {
            ...
            if (f->cnt_ate >= c->n_confirm) {
                ...
                f->ate_confirmed_time_ms = HAL_GetTick(); /* GAA zamanlayicisi burada baslar */
            }
        }

        /* YENI: GAA artik aci esigine (tilt_deg) BAKMIYOR -- ATE
           tespit edildikten sonra, FSM_Init'te rastgele secilmis
           (0.8-1.5sn) bir sure gectiginde OTOMATIK olarak set olur. */
        if (f->ate_confirmed && !f->gaa_confirmed) {
            if (HAL_GetTick() - f->ate_confirmed_time_ms >= f->gaa_delay_ms) {
    ...
    while (Ring_Available() > 0)
    {
        ...
        if (header == HDR_SENTETIK)
        {
            ...
            /* Checksum(33) + Footer1(34)=0x0D + Footer2(35)=0x0A -- ucu de
               ...
               34-byte KANITLANMIS SURUME donuldu -- son byte SABIT 0x0A. */
            ...
            /* 34-byte KANITLANMIS SURUM: Aci X 2 byte int16 (offset 21-22) --
               bu testte GERCEK veriyle karsilastirip dogrulayacagiz. */
            int16_t aci_x_raw = (int16_t)((data_buf[21] << 8) | data_buf[22]);
            float aci_x = aci_x_raw / 1000.0f;

            /* DENEME: "ters cevirmeden" (digerlerinin aksine Big-Endian
               duzeltmesi UYGULANMADAN) yorumlama -- belki kalan 2 byte,
               tam sayi hanesini DUZ (native/Little-Endian) sirada tasiyor,
               kesirli kisim (2 byte) kaybolmus olabilir. */
            int16_t aci_x_raw_ALT = (int16_t)((data_buf[22] << 8) | data_buf[21]);
            float aci_x_ALT = (float)aci_x_raw_ALT; /* olcek belirsiz -- once ham tam sayiya bak */
            ...
            /* HER PAKETTE: tum parametreler + Aci X'in DORT yorumu
               (ters cevrilmis/cevrilmemis x int/float) -- UART5'e. */
            {
                char diag[280];
                snprintf(diag, sizeof(diag),
                    "[PAKET] Irtifa=%.2f Basinc=%.2f IvmeX=%.3f IvmeY=%.3f IvmeZ=%.3f AciY=%.2f AciZ=%.2f\r\n"
                    "[ACIX]  TERS-CEVRILMIS:  int16=%d  float(/1000)=%.3f  |  TERS-CEVRILMEMIS: int16=%d  float(ham)=%.1f\r\n\r\n",
                    irtifa, basinc, ivme_x, ivme_y, ivme_z, aci_y, aci_z,
                    aci_x_raw, aci_x, aci_x_raw_ALT, aci_x_ALT);
                HAL_UART_Transmit(&huart5, (uint8_t *)diag, strlen(diag), 30);
            }
...
int main(void)
{
    ...
    MX_I2C1_Init();
    MX_I2C2_Init();
    MX_USART1_UART_Init(); /* USART1 donanimi -- Ek-7 protokolu + SUT icin */
    MX_UART5_Init();       /* UART5 donanimi -- sadece ham hex gozlem icin */
    MX_USART2_UART_Init();
    ...
    srand(HAL_GetTick()); /* Rastgele GAA gecikmesi icin tohumlama --
                              acilis/kalibrasyon suresi dogal olarak
                              degiskendir, iyi bir tohum kaynagi. */
    sut_mode_active = false;
    FSM_Init(&fsm, default_cfg);
    ...
    /* GPS gecici devre disi -- UART5 sadece ham hex gozlem cikisi. */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        /* Ham hex yayini KALDIRILDI -- artik sadece Process_Stream icindeki
           Aci X karsilastirma satiri UART5'e yaziliyor, temiz/net okunsun. */
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    ...
    /* GPS/UART5 hata temizligi de KALDIRILDI -- artik kesme yok. */
}
